  ...
  describe('cage stop', () => {
    describe('AC-1: Stop running server', () => {
      it('should stop a running server gracefully', async () => {
        ...
        // Mock process-utils functions with debug logging
        let isProcessRunningCallCount = 0;

        vi.mocked(isProcessRunning).mockImplementation(pid => {
          // First call returns true (process exists), second call returns false (process killed)
          return isProcessRunningCallCount++ === 0;
        });
        vi.mocked(killProcess).mockImplementation(() => true);
        let findProcessesCallCount = 0;
        vi.mocked(findProcessesOnPort).mockImplementation(() => {
          // First call: return a process to kill
          // Second call (after kill): return empty array (processes are dead)
          if (findProcessesCallCount++ === 0) {
            return [{ pid: 12345, name: 'node', command: 'cage server' }];
          }
          return [];
        });

        vi.mocked(killProcessesOnPort).mockImplementation(() => 1);

        let killCalled = false;
        vi.mocked(execSync).mockImplementation((cmd, _options) => {
          const cmdStr = String(cmd);
          if (cmdStr.includes('kill -0 12345')) {
            if (killCalled) {
              // After kill, process should be dead
              throw new Error('Process not found');
            }
            return Buffer.from(''); // Process exists initially
          }
          if (cmdStr.includes('kill -TERM 12345')) {
            killCalled = true;
            return Buffer.from(''); // Kill succeeds
          }
          return Buffer.from('');
        });
        ...
        expect(result.message).toContain('server stopped');
        // Changed expectation since we're now using killProcessesOnPort
        expect(killProcessesOnPort).toHaveBeenCalledWith(3790, undefined);
      });
    });
    ...
    describe('AC-11: Force stop server', () => {
      it('should force kill server with --force flag', async () => {
        ...
        const pidData = JSON.stringify({ pid: 12345, startTime: Date.now() });
        vi.mocked(readFileSync).mockImplementation((path, encoding) => {
          if (encoding === 'utf-8') return pidData;
          return Buffer.from(pidData);
        });
        vi.mocked(unlinkSync).mockReturnValue(undefined);

        // Mock process-utils functions
        vi.mocked(isProcessRunning)
          .mockReturnValueOnce(true)  // Line 85: Check if process exists
          .mockReturnValueOnce(false) // Line 94: Verify it's dead after kill
          .mockReturnValueOnce(false); // Extra safety
        vi.mocked(killProcess).mockReturnValue(true);
        vi.mocked(findProcessesOnPort)
          .mockReturnValueOnce([])  // No processes on port initially (PID from file is enough)
          .mockReturnValueOnce([]);  // After kill
        ...
        vi.mocked(execSync).mockImplementation((cmd, _options) => {
          ...
          if (cmdStr.includes('kill -KILL 12345')) {
            killCalled = true;
            return Buffer.from(''); // Kill succeeds
          }
          return Buffer.from('');
        ...
        expect(killProcess).toHaveBeenCalledWith(12345, true);
      });
    });
  });
  ...
  describe('cage status', () => {
    describe('AC-3: Status when server running', () => {
      it('should show running server status', async () => {
        ...
        vi.mocked(existsSync).mockReturnValue(true);
        const pidData = JSON.stringify({ pid: 12345, startTime: Date.now() - 60000 }); // 1 min uptime
        vi.mocked(readFileSync).mockImplementation((path, encoding) => {
          if (encoding === 'utf-8') return pidData;
          return Buffer.from(pidData);
        });
        vi.mocked(execSync).mockImplementation(cmd => {
          const cmdStr = String(cmd);
          if (cmdStr.includes('kill -0 12345')) return Buffer.from(''); // Process exists
          if (cmdStr.includes('curl') && cmdStr.includes('health'))
            return Buffer.from(''); // Health check
          if (cmdStr.includes('lsof')) return Buffer.from('node 12345'); // Port in use
          if (cmdStr.includes('ps')) return Buffer.from('12345 node 10:30'); // Process info
          return Buffer.from('');
        });
  ...
  describe('PID file management', () => {
    describe('AC-14: PID file creation and deletion', () => {
      ...
      it('should remove PID file on stop', async () => {
        ...
        vi.mocked(existsSync).mockReturnValue(true);
        vi.mocked(readFileSync).mockImplementation((path, encoding) => {
          if (encoding === 'utf-8') return '12345';
          return Buffer.from('12345');
        });
