...
const logger = new Logger({ context: 'DebugCommand' });
...
export async function appendDebugLog(
  ...
  try {
    ...
    logger.error('Failed to write to debug log:', { error });
  }
}
...
export function DebugMode({
  ...
  const remainingArgsRef = React.useRef(remainingArgs);
  const hasInitialized = React.useRef(false);

  // Update ref when remainingArgs changes
  React.useEffect(() => {
    remainingArgsRef.current = remainingArgs;
  }, [remainingArgs]);

  useEffect(() => {
    // Only run once to prevent infinite loop from store changes
    ...
    const initDebugMode = async () => {
      if (debugMode && logFile) {
        ...
        if (remainingArgsRef.current.length > 0) {
          await appendDebugLog(logFile, `Command: ${remainingArgsRef.current.join(' ')}`);
        }
    ...
    initDebugMode().catch(error => {
      logger.error('Failed to initialize debug mode:', { error });
    });

    // Track execution time on unmount
    return () => {
      if (debugMode && logFile && remainingArgsRef.current.length > 0) {
  ...
  }, [debugMode, logFile, enablePerformanceMonitoring, store]);
