...
export const StatusBar: React.FC<StatusBarProps> = ({ compact = false }) => {
  ...
  // Read directly from Zustand store - no polling needed, services handle updates
  const serverStatus = useAppStore(state => state.serverStatus);
  const hooksStatus = useAppStore(state => state.hooksStatus);
  const events = useAppStore(state => state.events);

  // Compute derived state
  const loading = serverStatus === 'unknown';
  ...
  // Get enabled hooks count
  const enabledHooks = hooksStatus?.installedHooks?.filter(h => h.enabled).length || 0;
  const totalHooks = hooksStatus?.installedHooks?.length || 0;

  // Compute status object for rendering logic
  const status = {
    server: {
      status: serverStatus,
      port: hooksStatus?.backendPort || 3790,
    },
    hooks: {
      installed: hooksStatus?.isInstalled || false,
      active: enabledHooks,
      total: totalHooks,
    },
    events: {
      total: totalEvents,
      today: eventsToday,
      rate: 0, // TODO: Calculate rate from recent events
    },
  };
