...
describe('FullScreenLayout', () => {
  ...
  // Helper to render with InputModeProvider
  const renderWithProvider = (component: React.ReactElement) => {
    return render(<InputModeProvider>{component}</InputModeProvider>);
  };
  ...
  describe('Scenario: Display full-screen layout structure', () => {
    it('Given a FullScreenLayout component When rendered Then it should display header, content area, and footer', () => {
      ...
      // Verify footer with navigation hints (default footer shows ESC)
  ...
  describe('Scenario: Clear screen on mount', () => {
    it('Given the layout component When it mounts Then it should render without clearing screen', () => {
      // FullScreenLayout doesn't clear the screen - that's handled by FullScreenWrapper
      const { lastFrame } = renderWithProvider(
      ...
      expect(lastFrame()).toContain('CAGE | Test');
  ...
  describe('Scenario: Consistent header and footer', () => {
    it('Given different views When using FullScreenLayout Then headers should be consistent', () => {
      ...
      // Both should have consistent footer with ESC
      expect(frame1()).toContain('ESC');
      expect(frame2()).toContain('ESC');
    });
  });
  ...
  describe('Scenario: Content area fills available space', () => {
    it('Given content in FullScreenLayout When rendered Then content area should use flexGrow', () => {
      ...
      const footerIndex = lines.findIndex(line => line.includes('ESC'));
