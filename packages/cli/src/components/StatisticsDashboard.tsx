...
export const StatisticsDashboard: React.FC<StatisticsDashboardProps> = ({ onBack }) => {
  ...
  const createBarChart = (data: Record<string, number>, maxWidth: number = 30): string[] => {
    ...
    return entries.slice(0, 10).map(([key, value]) => {
      ...
      const bar = '█'.repeat(fullChars) + '▓'.repeat(Math.max(0, 2)) + '░'.repeat(Math.max(0, emptyChars - 2));
      const percent = ((value / Object.values(data).reduce((a, b) => a + b, 0)) * 100).toFixed(1);
      return `${key.padEnd(20)} ${bar} ${formatNumber(value)} (${percent}%)`;
    });
  };

  const createTimelineChart = (data: Array<{ date: string; events: number }>): string[] => {
        ...
        {Object.entries(statistics.eventsByType).map(([type, count]) => {
          ...
          return (
            <Box key={type} flexDirection="column" marginBottom={1}>
              <Text bold>{type}</Text>
              <Text>  Total: {formatNumber(count)} ({formatPercentage(percentage)})</Text>
              <Text>  Average per day: {avgPerDay.toFixed(1)}</Text>
              <Text>  Trend: {trendSymbol} {formatPercentage(Math.abs(trend))}</Text>
            </Box>
          );
        })}
      </Box>
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        {Object.entries(statistics.hourlyDistribution).slice(8, 20).map(([hour, count]) => (
          <Text key={hour}>  {hour}:00 - {formatNumber(count)} events</Text>
        ))}
        <Text></Text>
        <Text bold>Peak Activity Periods:</Text>
        <Text>  Busiest Hour: {statistics.peakActivity.mostActiveHour.hour}:00 ({formatNumber(statistics.peakActivity.mostActiveHour.count)} events)</Text>
        <Text>  Peak Period: {statistics.peakActivity.busiestPeriod.start} - {statistics.peakActivity.busiestPeriod.end}</Text>
        <Text>  Peak Day: {statistics.peakActivity.mostActiveDay.date} ({formatNumber(statistics.peakActivity.mostActiveDay.count)} events)</Text>
      </Box>
    );
  };
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        {Object.entries(statistics.toolUsageStats).map(([tool, count]) => {
          const totalToolUsage = Object.values(statistics.toolUsageStats).reduce((a, b) => a + b, 0);
          const percentage = (count / totalToolUsage) * 100;

          return (
            <Text key={tool}>  {tool}: {formatNumber(count)} ({formatPercentage(percentage)})</Text>
          );
        })}
        <Text></Text>
  ...
  const renderPerformanceDetails = (): JSX.Element => {
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        <Text>  Fastest Tool: {statistics.performanceMetrics.fastestTool.name} ({statistics.performanceMetrics.fastestTool.avgTime}ms)</Text>
        <Text>  Slowest Tool: {statistics.performanceMetrics.slowestTool.name} ({formatNumber(statistics.performanceMetrics.slowestTool.avgTime)}ms)</Text>
        <Text></Text>
        ...
        <Text>  Session Reliability: {formatPercentage((1 - statistics.performanceMetrics.errorRate) * 100)}</Text>
      </Box>
    );
  };
      ...
      <Box flexDirection="column" flexGrow={1}>
        ...
        <Box flexDirection="column" marginBottom={1}>
          {SECTIONS.map((section, index) => {
            ...
            switch (section.key) {
              case 'events':
                const topEvent = Object.entries(statistics.eventsByType).sort((a, b) => b[1] - a[1])[0];
              ...
              case 'timeline':
                ...
                break;
              case 'tools':
                const topTool = Object.entries(statistics.toolUsageStats).sort((a, b) => b[1] - a[1])[0];
                ...
                return (
                  <Box flexDirection="column">
                    ...
                    {Object.entries(statistics.eventsByType).slice(0, 3).map(([type, count]) => {
                      const percentage = (count / statistics.totalEvents) * 100;
                      return <Text key={type}>{type}: {formatNumber(count)} ({formatPercentage(percentage)})</Text>;
                    })}
                  </Box>
                );
              ...
              case 'tools':
                return (
                  <Box flexDirection="column">
                    ...
                    {Object.entries(statistics.toolUsageStats).slice(0, 3).map(([tool, count]) => {
                      const totalToolUsage = Object.values(statistics.toolUsageStats).reduce((a, b) => a + b, 0);
                      const percentage = (count / totalToolUsage) * 100;
                      return <Text key={tool}>{tool}: {formatNumber(count)} ({formatPercentage(percentage)})</Text>;
                    })}
                    <Text>Tool Trends:</Text>
                    {Object.entries(statistics.trends.toolPopularityChange).slice(0, 3).map(([tool, change]) => {
                      const symbol = change > 0 ? '↗' : change < 0 ? '↘' : '→';
                      return <Text key={tool}>{symbol} {tool}: {change > 0 ? '+' : ''}{formatPercentage(change)}</Text>;
                    })}
                  </Box>
                );
              ...
              case 'performance':
                return (
                  <Box flexDirection="column">
                    ...
                    <Text>Average Response: {statistics.performanceMetrics.averageResponseTime}ms</Text>
                    <Text>Fastest: {statistics.performanceMetrics.fastestTool.name} ({statistics.performanceMetrics.fastestTool.avgTime}ms)</Text>
                    <Text>Slowest: {statistics.performanceMetrics.slowestTool.name} ({formatNumber(statistics.performanceMetrics.slowestTool.avgTime)}ms)</Text>
                    <Text>Error Rate: {formatPercentage(statistics.performanceMetrics.errorRate * 100)}</Text>
