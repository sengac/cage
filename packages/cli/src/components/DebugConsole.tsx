...
export const DebugConsole: React.FC<DebugConsoleProps> = ({ onBack }) => {
            ...
            lines.forEach(line => {
              try {
                ...
                events.push({
                  ...
                  message:
                    event.message || JSON.stringify(event.arguments || {}),
  ...
  const filteredEvents = debugEvents.filter(event => {
    ...
    if (filterComponent !== 'all' && event.component !== filterComponent)
      return false;
    ...
    if (appliedSearch) {
      const searchableContent = [
        ...
        .join(' ')
        .toLowerCase();
      if (!searchableContent.includes(appliedSearch.toLowerCase()))
        return false;
    }
  ...
  const components = Array.from(
    new Set(debugEvents.map(e => e.component))
  ).sort();
    ...
    (input, key) => {
      ...
      switch (input) {
        ...
        case 'f':
          ...
          const levels: Array<typeof filterLevel> = [
            'all',
            'DEBUG',
            'INFO',
            'WARN',
        ...
        case 'c':
          ...
          setFilterComponent(
            allComponents[(currentCompIndex + 1) % allComponents.length]
          );
          break;
        case 'r':
          // Clear all filters
        ...
        <Box flexDirection="column" width="100%">
          <Box width="100%">
            <Text color={textColor} wrap="truncate">
              {indicator} {time} {event.level.padEnd(5)}{' '}
              {event.component.substring(0, 15).padEnd(15)} {event.message}
