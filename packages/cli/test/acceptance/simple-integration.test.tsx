...
describe('CLI Integration Tests', () => {
  it('should show help when run with --help', async () => {
    try {
      const { stdout } = await execPromise('node dist/index.js --help');
      ...
      expect(error.stdout).toContain('Usage: cage');
    }
  });

  it('should show hooks help', async () => {
    try {
      const { stdout } = await execPromise('node dist/index.js hooks --help');
      expect(stdout).toContain('Manage Claude Code hooks');
      expect(stdout).toContain('setup');
      expect(stdout).toContain('status');
    } catch (error) {
      expect(error.stdout).toContain('Manage Claude Code hooks');
    }
  });

  it('should show events help', async () => {
    try {
      const { stdout } = await execPromise('node dist/index.js events --help');
      ...
      expect(error.stdout).toContain('Manage and view hook events');
    }
  });
});